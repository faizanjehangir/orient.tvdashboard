...
public class CustomAdapterVideoFileManager extends BaseAdapter {
	private Context mContext;
	LayoutInflater inflater;
	private ArrayList<VideoFileItem> items;

	public CustomAdapterVideoFileManager(Context context,
			ArrayList<VideoFileItem> items) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = inflater.inflate(
					R.layout.video_gridview_file_manager, null);
		...
		holder.image.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (items.get(i).getFile().isFile()) {
					...
					FragmentFileExplorer.resetGVAdapter(items.get(i).getFile());
				}
			}
		});
		if (position == 0) {
