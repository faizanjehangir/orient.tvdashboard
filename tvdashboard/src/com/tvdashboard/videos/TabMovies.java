...
import com.tvdashboard.videos.TabTVShows.getAllTvShows;
...
public class TabMovies extends FragmentActivity implements OnPageChangeListener {
	...
	public static List<Video> allMovies;
...
//        new getAllMovies().execute();
    ...
    private List<Fragment> getFragments(int numOfPages){
        ...
        {
        	f[i] = new FragmentMoviesMain().newInstance(String.valueOf(fragmentCounter));
        	fragmentCounter++;
        	fList.add(f[i]);
        }
    ...
    public class getAllMovies extends AsyncTask<Void, Void, Boolean>
	{
		...
		protected Boolean doInBackground(Void... params) {
			...
			try 
			{
				allMovies = dbHelper.getAllVideosByCategory("Movies");
				return true;
			}
			catch (Exception e)
			{
		...
		protected void onPostExecute(Boolean result) {			
			...
			if (result)
			{
				numOfPages = (int)Math.ceil(allMovies.size()/12.0);
		        ...
		        mViewPager.setOnPageChangeListener(TabMovies.this);
